<?php
session_start();
if(!isset($_SESSION["id"])){
  ?>
<script>
  document.location("login.php");
</script>
<?php
}
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>BARISAN & DERET</title>
    <link rel="stylesheet" href="css/style.css" />
  </head>
  <body style="background-image: url(images/background.png)" class="100vh">
    <div class="flex flex-col h-screen">
      <h1 class="text-3xl bg-hijau text-white font-bold mt-10 p-4 text-center">
        BARISAN & DERET
      </h1>
      <a href="./logout.php" class="ms-auto me-2 mt-2" style="background-color: red; color: white; padding: 5px 10px; border-radius: 10%; margin-top: 20px">Logout</a>
      <!--  end::navbar   -->
      <div class="flex flex-1 overflow-hidden mt-5 pb-5">
        <!--   start::Sidebar    -->
        <aside class="w-32 overflow-y-auto flex flex-col overflow-hidden">
          <a href="home.php" class="mb-2"
            ><img src="images/menu_icon/no_active/home.png" alt="home"
          /></a>
          <a href="materi.php" class="mb-2"
            ><img src="images/menu_icon/active/materi.png" alt="materi"
          /></a>
          <a href="contoh_soal.php" class="mb-2"
            ><img
              src="images/menu_icon/no_active/contoh_soal.png"
              alt="contoh_soal"
          /></a>
          <a href="quiz.php" class="mb-2"
            ><img src="images/menu_icon/no_active/quiz.png" alt="quiz"
          /></a>
          <a href="refleksi.php" class="mb-2"
            ><img src="images/menu_icon/no_active/refleksi.png" alt="refleksi"
          /></a>
          <a href="referensi.php" class="mb-2"
            ><img
              src="images/menu_icon/no_active/referensi.png"
              alt="referensi"
          /></a>
          <a href="tentang.php"
            ><img src="images/menu_icon/no_active/tentang.png" alt="tentang"
          /></a>
        </aside>

        <main class="text-white flex-1 ms-5 overflow-y-auto paragraph me-5 p-3">
          <div class="flex flex-col md:flex-row">
            <a href="materi/barisan_aritmatika.php"
              ><img
                src="images/menu_materi/barisan_aritmatika.png"
                alt="barisan_aritmatika"
            /></a>
            <a href="materi/barisan_geometri.php"
              ><img
                src="images/menu_materi/barisan_geometri.png"
                alt="barisan_geometri"
            /></a>
            <a href="materi/deret_aritmatika.php"
              ><img
                src="images/menu_materi/deret_aritmatika.png"
                alt="deret_aritmatika"
            /></a>
            <a href="materi/deret_geometri.php"
              ><img
                src="images/menu_materi/deret_geometri.png"
                alt="deret_aritmatika"
            /></a>
          </div>
        </main>
      </div>
    </div>
  </body>
</html>
